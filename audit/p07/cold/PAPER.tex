...
\title{Covering versus hitting in diversity embeddings:\newline an affine obstruction and matching bounds}
\author{Jeff Kline}
\date{Working note --- September 12, 2026}
\begin{document}
\maketitle
\begin{abstract}
For every prime power $q$, an explicit unit-length hypergraph on $q^2+1$
vertices requires $\ell_1$ diversity distortion between $q^2/(2q-1)$ and $q$,
despite an encoding of $O(q^3\log q)$ bits. This contradicts Conjecture 1.7
of Jozefiak and Shepherd, arXiv:2303.04199v1, and gives the same lower bound
for a connector-LP integrality gap. The principle is a separation between
covering cost and hitting probabilities, certified by incidence moments.
Conversely, universal set-cover approximations yield distortion at most
$4\sqrt n$ for unit-cost pairwise-intersecting supply hypergraphs, without
a common-root assumption. The square-root exponent is tight even when
the supply edges have empty global intersection. Positive costs give an
upper bound $2\sqrt{nH_n}$. A constant-factor coverage approximation for a
known greedy-bad family shows why that algorithm's logarithmic loss does
not itself establish a logarithmic obstruction to diversity embeddings.
\end{abstract}

\section{The covering--hitting principle}
A connected supply collection pays for the edges needed to cover its
terminals. A cut indicator asks only whether terminals occur on both sides
...
revision has not been checked. This working note records the proof without
...
numerical experiment. Agent reviews are not external peer review.
